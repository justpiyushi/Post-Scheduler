...
const Accounts = () => {
  ...
  const fetchAccounts = async (
    ...
    try {
      if (isSync) {
        ...
        await api.get("/api/v1/oauth/sync");
  ...
  useEffect(() => {
    ...
    const syncNeeded = params.get("sync");
  ...
  const handleConnect = async (platformId: string) => {
    ...
    try {
      const { data } = await api.get(`/api/v1/oauth/${platformId}/url`);
  ...
  const handleDisconnect = async (accountId: string) => {
    try {
      ...
      toast.success("Accounted disconnected successfully");
      await fetchAccounts();
    } catch (error: any) {
      toast.error(
        error?.response?.data?.message ||
    ...
    <div className="space-y-8 max-w-4xl">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 text-sm">
        ...
        <button
          ...
          className="flex item-center gap-2 px-5 py-2.5 bg-red-500 hover:bg-red-600 text-white rounded-full font-medium transition-all w-full sm:w-auto justify-center"
        >
